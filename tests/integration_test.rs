...
fn test_serialization_nanoserde() {
    ...
    assert_eq!(
        vec3,
        nanoserde::DeBin::deserialize_bin(&serialized_vec3).unwrap()
    );
    assert_eq!(
        quat,
        nanoserde::DeBin::deserialize_bin(&serialized_quat).unwrap()
    );
    assert_eq!(
        dquat,
        nanoserde::DeBin::deserialize_bin(&serialized_dquat).unwrap()
    );
}
...
fn test_serialization_speedy() {
    ...
    assert_eq!(
        quat,
        Quaternion::read_from_buffer(&serialized_quat).unwrap()
    );
    assert_eq!(
        dquat,
        DualQuaternion::read_from_buffer(&serialized_dquat).unwrap()
    );
}
